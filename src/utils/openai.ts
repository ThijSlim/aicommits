...
const getBasePrompt = (locale: string) =>
  `Write an insightful but concise Git commit message in a complete sentence in present tense for the diff that I provide you without prefacing it with anything, the response must be in the language ${locale}`;
...
export const generateCommitMessage = async (
  ...
  const commitMessageFormatPrompt = getCommitMessageFormatPrompt(type);

  const conventionalCommitsExtraContext =
    type === "conventional" ? getExtraContextForConventionalCommits() : "";
